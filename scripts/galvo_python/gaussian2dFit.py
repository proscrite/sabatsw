...
def fitGauss2d(im : np.ndarray, p0 : list):
    ...
    # test data
    g = gaussian2D(xx, yy, 1.2, 2.1, 0.5, 0.7, 1.1)
    ...
    fit = minimize(residuals, initial, args=(xx, yy, im))#, method='cg')
    return fit
...
def zoom_out(pars : list, bounds : list):
    ...
    def fit2dGauss(im : np.ndarray, DEBUG : bool = False) :
        """Fit 2d gaussian to an RGB image, perform ROI selection, parameter setting"""
        img = rgb2gray(im)
        mx = guess_centroid(img)
        bounds = bound_roi(img, mx, 500)
        imroi = select_roi(img, bounds)
